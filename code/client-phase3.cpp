...
class ClientSide
{
    ...
    ClientSide(char* p, char* p2)
    {
        ...
        while(configfile)
        {
            ...
            {
                size_t pos = 0;
                while(isalpha(text[text.size()-1])==0) 
                {
    ...
    void doClientStuff(char* p, char* p2)
    {
        ...
        for(int i=0;i<numberneighbours;i++)
        {
            ...
            for(int j=0;j<numbersearchfiles;j++)
            {
                ...
                send(usock[i] , f , strlen(f) , 0 );
                valread = 0;
                while(valread==0){
                    valread = read( usock[i] , buffer2, 1024);
                }
                if(buffer2[0]=='Y')
                {
            ...
            send(usock[i] , confirm , strlen(confirm) , 0 );
        ...
        for(int i=0;i<numberneighbours;i++)
        {
            ...
            for(int j=0;j<numbersearchfiles;j++)
            {
                if(nums[j]==-1)
                {
                    ...
                    {
                        printed[j] = true;
                        cout<<"Found "<<searchfiles[j]<<" at 0 with MD5 0 at depth 0"<<endl;
                    }
                    else
                    continue;
                }
                ...
                {
                    send(sock[i], searchfiles[j].c_str(), strlen(searchfiles[j].c_str()), 0);
                    ...
                    while(1){
                        char buffer100[SIZE];
                        n = recv(sock[i],buffer100,SIZE, 0);
                        if (n <= 0){
    ...
    ServerSide(char* p, char* p2)
    {
        ...
        if ((dir = opendir(p2)) != nullptr) 
        {
            while ((diread = readdir(dir)) != nullptr) 
            {
                ...
                if((diread->d_name)[0]!='.' && !((diread->d_name)[0]=='D' && (diread->d_name)[2]=='w' && (diread->d_name)[4]=='l'))
                {
                    files.push_back(diread->d_name);
                    string xx = diread->d_name;
                    ownfiles.push_back(xx);
